...
Его представляет абстрактный класс `Model<T>` и реализующий его `AppState`, отвещающий за хранение ключевых данных o всех товарах и о заказе, необходимые для взаимодействия с сервером и осуществеления бизнес логики.
...
Его представляет абстрактный класс `Component<T>`, который реализуется в нескольких классах - `Page`, `Modal`, `ItemCard`, `Form<T>`, `Basket`, `Success`. Классы осуществляют создание визуального образа приложения в виде различных компонентов и обработку пользовательских событий.
